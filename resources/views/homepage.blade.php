<p>Testing page</p>

{{-- 4154f1 --}}

<a href='/staffs/dashboard'>Dashboard</a><br/>
<a href="/staffs/students/viewStudent" class="btn btn-primary">View all Student</a>
<a href="/students/create" class="btn btn-primary">Create Student</a><br/>
<a href="/students/pendingEmailVerify" class="btn btn-primary">Pending Email Verification</a><br/>
<a href="/students/successEmailVerify" class="btn btn-primary">Success Email Verification</a><br/>
